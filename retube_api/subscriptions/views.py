        ...
        try:
            ...
            if customer_data:
                ...
                session = stripe.checkout.Session.create(
                    success_url=f'{env("CLIENT_URL")}/account?session_id={{CHECKOUT_SESSION_ID}}',
                    cancel_url=f'{env("CLIENT_URL")}/account/',
                    mode="subscription",
    ...
    def post(self, request, format=None):
        try:
            ...
            session = stripe.billing_portal.Session.create(
                customer=customer_id,
                return_url=f'{env("CLIENT_URL")}/account/',
            )
    ...
    def post(self, request, format=None):
        ...
        if event_type == "checkout.session.completed":
            ...
            subscription_obj = Subscription.objects.get(
                user__email=data_object.customer_details.email
            )
            ...
            stripe_subscription = stripe.Subscription.retrieve(subscription_id)
            subscription_obj = Subscription.objects.get(
                user__email=data_object.customer_email
            )
...
class PlansView(APIView):
    """
    Fetch the details for all subscription plans.
    """
    ...
    def get(self, request, format=None):
        ...
        plan_serializer = SubscriptionPlanSerializer(

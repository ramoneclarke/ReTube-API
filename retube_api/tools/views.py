...
class VideoSnippetView(APIView):
    ...
    def get(self, request, format=None):
        ...
        snippet_serializer = SnippetSerializer(
    ...
    def post(self, request, format=None):
        ...
        if usage >= limit:
            return Response(
                {"detail": "Snippet limit exceeded for current subscription level"},
        ...
        if Snippet.objects.filter(
            owner=user, video__video_id=video_id, start=start, end=end
        ).exists():
...
class SummaryView(APIView):
    ...
    def get(self, request, format=None):
        user = request.user
        summaries = Summary.objects.filter(owner=user).order_by("-id")
        self.check_object_permissions(request, summaries)
    ...
    def post(self, request, format=None):
        ...
        if usage + 1 >= limit:
            return Response(
                {"detail": "Summary limit exceeded for current subscription level"},
                status=status.HTTP_403_FORBIDDEN,
            )
        ...
        if YoutubeVideo.objects.filter(video_id=video_id).exists():
            ...
            if Summary.objects.filter(video=video).exists():
                ...
                summary = create_summary(video_id, user)
        else:
            summary = create_summary(video_id, user)
